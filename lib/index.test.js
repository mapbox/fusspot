...
describe('v.strictShape', () => {
  test('invalid properties are rejected', () => {
    var check = t(v.strictShape({ name: v.string }));

    expect(check({ foo: 3 })).toEqual(['The following keys are invalid: foo']);

    expect(check({ foo: 3, bar: 'thing' })).toEqual([
      'The following keys are invalid: foo, bar'
    ]);

    expect(check({ foo: 3, name: 'horse', bar: 3, baz: true })).toEqual([
  ...
  test('regular shape validation', () => {
    var check = t(v.strictShape({ name: v.required(v.string), good: v.boolean }));
