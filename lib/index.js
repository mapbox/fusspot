...
v.objectOf = function objectOf(validator) {
  return function objectOfValidator(value) {
    var validationResult = validate(v.plainObject, value);
    ...
    objectEntries(value).forEach(function(entry) {
      validationResult = validate(validator, entry.value);
      if (validationResult) {
        errorMessages.push([entry.key].concat(validationResult));
      }
    });
...
v.strictShape = function strictShape(validatorObj) {
  ...
  return function strictShapeValidator(value) {
    ...
    if (
      ...
      Object.keys(validatorObj).length > 0
    ) {
      return function() {
        return (
          'The following keys are required: ' +
          Object.keys(validatorObj).join(', ')
        );
      };
    }
